...
class Actor(nn.Module):
    ...
    def forward(self, state, rho, h, c):
        ...
        raw_action = torch.tanh(self.Linear2(x))   # [-1, 1]
        action = 0.5 * (raw_action + 1.0)      # [0, 1]
...
class Critic(nn.Module):
    ...
    def forward(self, state, action, rho,h, c):
        x = torch.cat([state, action,rho],dim=-1)
        x = F.relu(self.Linear1(x))
        # LSTM
        x, (new_h, new_c) = self.lstm(x, (h, c))
    ...
    def train(self, env,
        ...
        for episode in iterator:
            ...
            while not done:
                ...
                # Get action
                a, h_actor, c_actor = self.get_action(torch.from_numpy(obs).float().to(device).unsqueeze(0).unsqueeze(0), torch.tensor(rho).float().unsqueeze(0).unsqueeze(0), 
                ...
                with torch.no_grad():
                    q_values, h_critic, c_critic = self.get_value(torch.from_numpy(obs).float().to(device).unsqueeze(0).unsqueeze(0), torch.from_numpy(a).float().to(device).unsqueeze(0).unsqueeze(0),torch.tensor(rho).float().unsqueeze(0).unsqueeze(0), h_critic, c_critic)
