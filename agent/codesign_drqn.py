...
class Q_net(nn.Module):
    ...
    def sample_action(self, x, rho, h, c, epsilon):
        output = self.forward(x, rho, h, c)
...
class CodesignDRQNagent:
    def __init__(self, observation_space,action_space,rho_space,learning_rate,buffer_len,lookup_step,gamma,
                      batch_size,tau,eps_start,eps_decay, eps_end,random_update,min_epi_num,target_update_period): 
        
        
        #critic setting
        self.critic = Q_net(observation_space,action_space,rho_space).to(device)
    ...
    def train(self, env, 
             ...
             sigma = np.array([0.2, 0.2], dtype=float),
             cost_per_kwh_max=20*150,
             cost_per_kw_max=30*150,
        ...
        for episode in iterator:
            ...
            rho = np.array([rho_E,rho_P])
            obs = env.reset(rho_E,rho_P)
            done = False
            ...
            while not done:
                ...
                # Get action
                a, h, c = self.critic.sample_action(torch.from_numpy(obs).float().to(device).unsqueeze(0).unsqueeze(0), torch.tensor(rho).float().unsqueeze(0).unsqueeze(0),
                                                  h.to(device), c.to(device),
                                                  epsilon)

                # Do action
                s_prime, r, done = env.step(a,rho)
                obs_prime = s_prime
                episode_bidding.append(env.bidding)  # Collect bidding data
                
                with torch.no_grad():
                    episode_q0, _, _ = self.critic(torch.from_numpy(obs).float().to(device).unsqueeze(0).unsqueeze(0), torch.tensor(rho).float().unsqueeze(0).unsqueeze(0),h, c)
          ...
          # Save episode bidding data
            ...
            if episode >= min_epi_codesign:
                 if episode % 10 ==0:
                    ...
                    capex_total = rhos_E * cost_per_kwh + rhos_P * cost_per_kw
                    G = np.array(reward_list[-10:])*365/7 -  capex_total
                    rhos = np.stack([rhos_E, rhos_P], axis=1)
            ...
            if LOG_DIR:
                ...
                summary_writer.add_scalar("cost_per_kwh",     cost_per_kwh,     episode)
                summary_writer.add_scalar("cost_per_kw",     cost_per_kw,     episode)
                summary_writer.add_scalar("mu_E",     mu[0],     episode)
                summary_writer.add_scalar("mu_P",     mu[1],     episode)
    ...
    def update_step(self):
        ...
        with torch.no_grad():
            q_target, _, _ = self.critic_target(next_observations, rhos,
        ...
        q_out, _, _ = self.critic(observations, rhos,h.to(device), c.to(device))
