...
class DRDPGagent:
    ...
    def train(self, env,
        ...
        for episode in iterator:
            ...
            while not done:
                ...
                episode_bidding.append(env.bidding_pv)  # Collect bidding data
                episode_AGC.append(env.AGC_reg)
                episode_a_reg_fcas.append(env.a_fcas_reguration)
                episode_a_con_fcas.append(env.a_fcas_contingency)
            ...
            AGC_history.append(episode_AGC)
            a_reg_fcas_history.append(episode_a_reg_fcas)
            a_con_fcas_history.append(episode_a_con_fcas)
        ...
        AGC_df = pd.DataFrame(AGC_history)
        fcas_reg_df = pd.DataFrame(a_reg_fcas_history)
        fcas_con_df = pd.DataFrame(a_con_fcas_history)
        
        # Save each DataFrame to CSV files
        bidding_df.to_csv(f'action/episode_bidding_{seed}_{current_datetime}.csv', index=False)
        AGC_df.to_csv(f'action/episode_AGC_{seed}_{current_datetime}.csv', index=False)
        fcas_reg_df.to_csv(f'action/episode_fcas_reg_{seed}_{current_datetime}.csv', index=False)
        fcas_con_df.to_csv(f'action/episode_fcas_con_{seed}_{current_datetime}.csv', index=False)
    ...
    def update_step(self):
        ...
        next_actions, _, _ = self.actor_target(next_observations, h_actor_target, c_actor_target)
        
        # ターゲットクリティックネットワークでQ値を計算
        target_q_values, _, _ = self.critic_target(next_observations, next_actions, h_critic_target, c_critic_target)
        ...
        current_q_values, _, _ = self.critic(observations, actions, h_critic, c_critic)
        ...
        if self.policy_update_cnt == 0:
            ...
            predicted_actions, h_actor_next, c_actor_next = self.actor(observations, h_actor, c_actor)
            actor_loss = -self.critic(observations, predicted_actions, h_critic, c_critic)[0].mean()
