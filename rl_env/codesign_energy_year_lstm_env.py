...
class MultiyearEnv:
    def __init__(self,agent_idx,mode):
        ...
        self.price_energy_all   = self.market_data.iloc[:, 1].to_numpy()
        self.price_sr_all       = self.ancillary_data.iloc[:, 2].to_numpy()
        self.price_up_all       = self.ancillary_data.iloc[:, 3].to_numpy()
        self.price_dn_all       = self.ancillary_data.iloc[:, 4].to_numpy()
        ...
        self.results_path = os.path.join(
            results_dir, f"episode_results_{mode}_{now}.csv"
        )
    ...
    def step(self, action):
            ...
            if self.agent_idx == 'all':
               reward = (revenue_energy + revenue_fcas + capacity_payment +  imbalance_revenue - degradation_cost) / 10000
            else:
                reward =  (revenue_energy + revenue_fcas + capacity_payment +  imbalance_revenue - degradation_cost) * 365 / self.days_per_month[self.agent_idx] / 10000
