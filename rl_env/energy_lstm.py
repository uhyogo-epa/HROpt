...
class EnergyLSTMEnv:
    def __init__(self,mode = 'discrete', battery_times = 1,learning_rate = 0.1):
        ...
        self.observation_space = self.hidden_space*2 + self.state_space  # 充放電の状態, 日射量, 市場価格をobservationとする
    ...
    def step(self, action):
        ...
        if self.current_step >= len(self.solar_radiation):
            ...
            observation = np.zeros_like(self.reset())  # ダミー観測値
        else:
            observation = np.concatenate((
                np.array([self.soc, current_solar_radiation, current_market_price], dtype=np.float32),
                h_np,
                c_np
            ))
